...
if not st.session_state.logged_in:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📦 Import CSV", "🛍️ Products", "👥 Customers", "📝 Quotes"])
    
    with tab1:
        st.header("Import Products to Database")
        st.info("💡 **Smart Import:** If your CSV already has 'AI_B2B_Description' and 'Sell_Price_AUD' columns, it will import instantly without using API credits!")
        uploaded_file = st.file_uploader("Upload CSV (Raw 1688 or Pre-Cleaned)", type="csv")
        
        if uploaded_file:
            df = pd.read_csv(uploaded_file)
            st.write("Preview:", df.head())
            
            has_ai = "AI_B2B_Description" in df.columns
            has_price = "Sell_Price_AUD" in df.columns
            ...
            if st.button("🚀 Import to Database", type="primary"):
                if "标题" not in df.columns:
                    ...
                    for i, row in df.iterrows():
                        ...
                        # SMART CHECK: Use existing data if available, otherwise generate
                        if has_ai and has_price:
                            desc = str(row["AI_B2B_Description"])
                            price_aud = float(row["Sell_Price_AUD"])
                        else:
                        ...
                        img = str(row.get("offer-img", ""))
                        link = str(row.get("标题链接", ""))
    ...
    with tab2:
        ...
        if products:
            ...
            st.info("No products in database yet. Use the Import tab.")
